...
export function PlayerController() {
  ...
  const submitTokens = () => {
      // Check if taking these tokens will exceed 10
      if (totalTokens + selectedTokens.length > 10) {
          showConfirm(t('Taking these tokens will exceed the 10-token limit. You will need to discard some. Proceed?'), () => {
      ...
      <div className="flex flex-col h-full">
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <div className="flex items-center justify-center bg-black/30 rounded-xl p-3 min-h-[80px]">
                          <div className="flex gap-4">
                              {['ruby', 'emerald', 'sapphire', 'diamond', 'onyx', 'gold'].map((color) => {
                                  const count = gameState.lastAction?.tokens?.[color as TokenColor];
                                  return count && count > 0 ? (
                                      <Token key={color} color={color as TokenColor} count={count} size="lg" />
                                  ) : null;
                              })}
                          </div>
                      </div>
          ...
          <div className="mx-2 mb-6 px-6 py-4 bg-slate-800/80 backdrop-blur-sm rounded-2xl border border-slate-700 shadow-xl relative">
               ...
               <div className="flex gap-4 justify-center">
                   {['ruby', 'emerald', 'sapphire', 'diamond', 'onyx', 'gold'].map(c => (
                       <div key={c} className="flex flex-col items-center">
                           <div className={clsx("w-10 h-10 rounded-full border-2 border-gray-500 overflow-hidden shadow-md", GEM_BORDER_COLORS[c as GemColor] || 'border-yellow-600')}>
                                <img src={GEM_IMAGES[c as GemColor]} className="w-full h-full object-cover scale-150" />
                           </div>
                           <span className="text-base font-bold text-gray-200 mt-2">{gameState.board.tokens[c as TokenColor] || 0}</span>
                       </div>
                   ))}
               </div>
          </div>
          ...
          <div className="flex justify-between items-center mb-6 px-4 mt-2">
              ...
              <div className="flex items-center gap-6">
                  <div className="flex bg-slate-800 rounded-xl border border-slate-700 p-1">
                      <button onClick={() => changeLanguage('en')} className={`px-3 py-1.5 text-sm rounded-lg transition-all font-bold ${i18n.language === 'en' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-slate-700'}`}>EN</button>
                      <button onClick={() => changeLanguage('ja')} className={`px-3 py-1.5 text-sm rounded-lg transition-all font-bold ${i18n.language === 'ja' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-slate-700'}`}>JA</button>
                  </div>
          ...
          <div className="flex-1 px-4 mb-24 overflow-y-auto custom-scrollbar">
              <div className="flex items-baseline justify-between mb-6 border-b border-gray-700 pb-3">
                  ...
                  <div className={clsx("text-base font-bold px-4 py-1.5 rounded-full border shadow-inner", totalTokens > 10 ? "bg-red-900/50 border-red-500 text-red-100" : "bg-slate-800 border-slate-600 text-slate-200")}>
      ...
      <div className="space-y-4">
          ...
          <div className="bg-gray-800 p-4 rounded-xl">
             ...
             <div className="flex flex-wrap gap-4 justify-center mb-6">
                {(['ruby', 'emerald', 'sapphire', 'diamond', 'onyx'] as TokenColor[]).map(color => (
      ...
      return (
          <div className="space-y-4 p-4 bg-red-900/20 rounded-xl border border-red-500/50">
               ...
               <div className="space-y-2">
                   {['diamond', 'sapphire', 'emerald', 'ruby', 'onyx', 'gold'].map((color) => {
          ...
          >
            <div className="flex flex-col items-center gap-6">
                <div className="text-2xl font-serif text-blue-200">
                    {isWinner ? <span className="text-green-400 animate-pulse">{t('You Won!')}</span> : <span>{winner?.name} {t('Won')}</span>}
                </div>

                <div className="flex flex-col gap-2 w-full">
                    <h3 className="text-lg font-bold text-slate-400 uppercase tracking-wider border-b border-slate-700 pb-1 mb-2">{t('Final Standings')}</h3>
                    {gameState.players.sort((a,b) => b.score - a.score).map((p, i) => (
                        <div key={p.id} className="flex justify-between items-center p-3 rounded-xl bg-slate-700/30 hover:bg-slate-700/50 transition">
                            <div className="flex items-center gap-4">
                                 <span className={clsx("font-bold text-lg w-6", i===0 ? "text-amber-400" : "text-slate-500")}>{i+1}</span>
                                 <span className={clsx("font-bold text-lg", p.id === playerId ? "text-yellow-400" : "text-slate-300")}>{p.name}</span>
                            </div>
                            <span className="font-mono text-xl font-bold text-amber-400">{p.score}</span>
                        </div>
                    ))}
                </div>

                <div className="flex gap-4 w-full pt-4">
                    <button onClick={() => setShowResults(false)} className="flex-1 py-4 bg-slate-700 hover:bg-slate-600 text-white font-bold rounded-xl shadow-lg transition-all">
                        {t('View Dashboard')}
                    </button>
                    <button onClick={() => navigate('/')} className="flex-1 py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg transition-all">
                        {t('Back to Home')}
                    </button>
                </div>
            </div>
